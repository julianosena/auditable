...
import br.com.zup.itau.auditable.repository.jql.FilterDefinition.*;
...
import static br.com.zup.itau.auditable.common.collections.Lists.asList;
import static br.com.zup.itau.auditable.repository.jql.InstanceIdDTO.instanceId;
import static java.time.LocalTime.MIDNIGHT;
import static br.com.zup.itau.auditable.repository.jql.ShadowScope.*;
...
public class QueryBuilder {
    ...
    /**
     * Limits the number of Snapshots to be fetched from ItauAuditableRepository in a single query.
     ...
     *
     * Since all of ItauAuditable queries rely on <b>Snapshots</b>
     * in order to generate their output, the limit filter affects all of them,
     * but in a different way:
     *
     * <ul>
     *   <li>{@link ItauAuditable#findSnapshots(JqlQuery)} &mdash; the limit works intuitively,
     *   it's the maximum size of a returned list.
     *   </li>
     *   <li>{@link ItauAuditable#findChanges(JqlQuery)} &mdash;
     ...
     *   </li>
     *   <li>{@link ItauAuditable#findShadows(JqlQuery)} &mdash;
     ...
     *   </li>
     *   <li>{@link ItauAuditable#findShadowsAndStream(JqlQuery)} &mdash;
     *   the resulting stream is <b>lazily loaded</b> and it's limited only by
     *   the size of your ItauAuditableRepository and your heap.
     *   When the limit is hit, ItauAuditable repeats a given query to load a next bunch of Snapshots.
     *   Shadow graphs loaded by findShadowsAndStream() are always complete,
     *   but can trigger a lot of queries.
     *   </li>
     * </ul>
     *
     * <b>We recommend</b> {@link ItauAuditable#findShadowsAndStream(JqlQuery)}
     ...
     * <br/><br/>
     *
     * For paging Shadows use {@link ItauAuditable#findShadowsAndStream(JqlQuery)}
     ...
     * <br/><br/>
     *
     * Read more about query scopes in {@link ItauAuditable#findShadows(JqlQuery)} javadoc.
     * <br/><br/>
     *
     ...
     * in {@link ItauAuditable#findShadows(JqlQuery)} javadoc.
     * <br/><br/>
     *
     * Only for Shadow queries.
     *
