...
class ObjectGraphBuilder {
    ...
    private void buildSingleEdges(LiveNode node) {
        for (ItauAuditableProperty singleRef : getSingleReferencesWithManagedTypes(node.getManagedType())) {
    ...
    private void buildMultiEdges(LiveNode node) {
        ...
        for (ItauAuditableProperty containerProperty : getNonEmptyEnumerablesWithManagedTypes(node))  {
    ...
    private List<ItauAuditableProperty> getSingleReferencesWithManagedTypes(ManagedType managedType) {
        return managedType.getProperties(property -> property.getType() instanceof ManagedType);
    }

    private List<ItauAuditableProperty> getNonEmptyEnumerablesWithManagedTypes(final LiveNode node) {
