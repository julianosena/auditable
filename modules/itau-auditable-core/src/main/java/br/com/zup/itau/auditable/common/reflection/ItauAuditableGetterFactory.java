 ...
 */
class ItauAuditableGetterFactory {
    private final Class getterSource;
    private final List<ItauAuditableGetter> getters = new ArrayList<>();
    private final TypeResolvingContext context = new TypeResolvingContext();

    ItauAuditableGetterFactory(Class getterSource) {
     ...
     */
    List<ItauAuditableGetter> getAllGetters() {
        if (getters.size() > 0) {
    ...
    private void findAllGetters(Class currentGetterSource) {
        ...
        while (clazz != null && clazz != Object.class) {
            ...
            Arrays.stream(clazz.getDeclaredMethods())
                    ...
                    .map(getter -> createItauAuditableGetter(getter, context))
    ...
    private boolean excludeDuplicatedProperties(ItauAuditableGetter getter, List<ItauAuditableGetter> getters) {
        final String propertyName = getter.propertyName();
        return getters.stream()
                .map(ItauAuditableGetter::propertyName)
    ...
    private static boolean isOverridden(Method parent, Collection<ItauAuditableGetter> toCheck) {
    ...
    private ItauAuditableGetter createItauAuditableGetter(Method getterMethod, TypeResolvingContext context) {
        Type actualReturnType = context.getSubstitution(getterMethod.getGenericReturnType());

        if (hasInheritedId(getterMethod)) {
            return new ItauAuditableGetter(getterMethod, actualReturnType, true);
        }
        return new ItauAuditableGetter(getterMethod, actualReturnType);
    }
