...
class ShadowGraphBuilder {
    ...
    private Object passValueOrCreateNodeFromRef(Object value, ItauAuditableProperty property) {
    ...
    private ShadowBuilder createOrReuseNodeFromRef(GlobalId globalId, ItauAuditableProperty property) {
        ...
        if (property.isShallowReference()) {
            EntityType shallowReferenceType = property.getType() instanceof EntityType
                    ? property.getType()
                    : (EntityType)typeMapper.getItauAuditableManagedType(globalId);
