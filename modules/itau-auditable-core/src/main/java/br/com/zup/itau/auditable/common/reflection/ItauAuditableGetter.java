 ...
 */
public class ItauAuditableGetter extends ItauAuditableMember<Method> {
    private static final Logger logger = getLogger(ItauAuditableGetter.class);

    private static final Object[] EMPTY_ARRAY = new Object[]{};

    private final Optional<Method> setterMethod;

    protected ItauAuditableGetter(Method getterMethod, Type resolvedReturnType) {
        super(getterMethod, resolvedReturnType);
        setterMethod = findSetterForGetter(getterMethod);
    }

    protected ItauAuditableGetter(Method getterMethod, Type resolvedReturnType, boolean looksLikeId) {
    ...
    public void setEvenIfPrivate(Object onObject, Object value) {
        setterMethod.orElseThrow(() -> new ItauAuditableException(ItauAuditableExceptionCode.SETTER_NOT_FOUND,
                getRawMember().getName(), getRawMember().getDeclaringClass().getName()));

        try {
            setterMethod.get().invoke(onObject, value);
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ItauAuditableException(ItauAuditableExceptionCode.SETTER_INVOCATION_ERROR,
