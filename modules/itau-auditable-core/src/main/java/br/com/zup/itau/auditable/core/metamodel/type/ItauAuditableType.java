 ...
 */
public abstract class ItauAuditableType {
    ...
    ItauAuditableType(Type baseJavaType) {
        this(baseJavaType, Optional.empty(), 0);
    }

    ItauAuditableType(Type baseJavaType, Optional<String> name, int expectedArgs) {
     ...
     */
    ItauAuditableType spawn(Type baseJavaType) {
        try {
            Constructor c = this.getClass().getConstructor(Type.class);
            return (ItauAuditableType)c.newInstance(new Object[]{baseJavaType});
    ...
    /**
     * ItauAuditableType name, clientsClass.name by default
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ItauAuditableType)) return false;

        ItauAuditableType that = (ItauAuditableType) o;
