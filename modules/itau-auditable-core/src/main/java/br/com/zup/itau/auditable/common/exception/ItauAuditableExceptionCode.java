...
/**
 * Enums with all ItauAuditable errors codes
 *
...
public enum ItauAuditableExceptionCode {
    ...
    CLASS_EXTRACTION_ERROR(ItauAuditableException.BOOTSTRAP_ERROR + "Can't extract Class from Type '%s'.") ,
    ...
    COMPARING_TOP_LEVEL_VALUES_NOT_SUPPORTED("Comparing top-level %ss like '%s' is not supported. ItauAuditable.compare() is designed to deeply compare two arbitrary complex object graphs. For simple values, equals() does the job."),
    ...
    ALREADY_BUILT(ItauAuditableException.BOOTSTRAP_ERROR + "instance already built, each AbstractContainerBuilder may produce only one target instance"),
    ...
    CONTAINER_NOT_READY(ItauAuditableException.BOOTSTRAP_ERROR +"pico container is not ready"),
    ...
    SNAPSHOT_NOT_FOUND("snapshot '%s' not found in ItauAuditableRepository"),
    ...
    TYPE_NAME_NOT_FOUND(
            ...
            "using ItauAuditableBuilder.withPackagesToScan(String) or ItauAuditableBuilder.scanTypeName(Class)"),
    ...
    COMPONENT_NOT_FOUND(ItauAuditableException.BOOTSTRAP_ERROR+"component of type '%s' not found in container") ,
    ...
    CANT_DELETE_OBJECT_NOT_FOUND("failed to delete object {'%s'}, "+
            "it doesn't exists in ItauAuditableRepository"),

    CANT_FIND_COMMIT_HEAD_ID("can't find commit head id in ItauAuditableRepository"),
    ...
    ItauAuditableExceptionCode(String message) {
