...
import br.com.zup.itau.auditable.core.diff.DiffAssert
...
import br.com.zup.itau.auditable.core.metamodel.type.IgnoredType
...
import static br.com.zup.itau.auditable.core.model.DummyUser.Sex.MALE
import static br.com.zup.itau.auditable.core.model.DummyUser.dummyUser
import static br.com.zup.itau.auditable.core.model.DummyUserWithPoint.userWithPoint

/**
 * @author bartosz walacik
 */
class ItauAuditableDiffE2ETest extends AbstractDiffTest {
    ...
    def "should allow passing null to currentVersion"(){
      ...
      when:
      def diff = itauAuditable.compare(new SnapshotEntity(id:1), null)
    ...
    def "should allow passing null to oldVersion"(){
        ...
        when:
        def diff = itauAuditable.compare(null, new SnapshotEntity(id:1))
    ...
    def "should allow passing two nulls to compare()"(){
        ...
        when:
        def diff = itauAuditable.compare(null, null)
    ...
    def "should ignore all props of #classType which are not in the 'included' list of properties"(){
      given:
      def itauAuditable = ItauAuditableBuilder.itauAuditable().registerType(definition).build()
    ...
    def "should extract Property from PropertyChange"(){
      given:
      def itauAuditable = ItauAuditableTestBuilder.newInstance()

      when:
      def diff = itauAuditable.compare(new Person('1','bob'), new Person('1','bobby'))
      PropertyChange propertyChange = diff.changes[0]

      Property property = itauAuditable.getProperty( propertyChange )
    ...
    def "should create NewObject for all nodes in initial diff"() {
        ...
        when:
        def diff = itauAuditable.initial(left)
    ...
    def "should create properties snapshot of NewObject only when configured"() {
        given:
        def itauAuditable = ItauAuditableBuilder.itauAuditable().withNewObjectsSnapshot(true).build()
        def left =  new DummyUser(name: "kazik")
        def right = new DummyUser(name: "kazik", dummyUserDetails: new DummyUserDetails(id: 1, someValue: "some"))
    ...
    def "should create valueChange with Enum" () {
        ...
        when:
        def diff = itauAuditable.compare(user, user2)
    ...
    def "should serialize whole Diff"() {
        ...
        when:
        def diff = itauAuditable.compare(user, user2)
        def jsonText = itauAuditable.getJsonConverter().toJson(diff)
    ...
    def "should support custom JsonTypeAdapter for ValueChange"() {
        given:
        def itauAuditable = itauAuditable().registerValueTypeAdapter( new DummyPointJsonTypeAdapter() )
                             .build()

        when:
        def diff = itauAuditable.compare(userWithPoint(1,2), userWithPoint(1,3))
        def jsonText = itauAuditable.getJsonConverter().toJson(diff)

        then:
        def json = new JsonSlurper().parseText(jsonText)
        def change = json.changes[0];
        change.globalId.valueObject == "br.com.zup.itau.auditable.core.model.DummyUserWithPoint"
    ...
    def "should support custom native Gson TypeAdapter"() {
        given:
        def itauAuditable = itauAuditable()
    ...
    def "should understand primitive default values when creating NewObject snapshot"() {
        ...
        when:
        def diff = itauAuditable.initial(new PrimitiveEntity())
    ...
    def "should understand primitive default values when creating ValueChange"() {
        ...
        when:
        def diff = itauAuditable.compare(new PrimitiveEntity(), new PrimitiveEntity())
    ...
    def "should serialize the Diff object"() {
        ...
        ageChange.affectedGlobalId.typeName == "br.com.zup.itau.auditable.core.model.DummyUser"
    ...
    def "should compare ShallowReferences using regular ReferenceChange"() {
        ...
        when:
        ReferenceChange change = itauAuditable.compare(left, right).changes.find{it instanceof  ReferenceChange}
    ...
    def "should not compare properties when class is mapped as ShallowReference"() {
        ...
        expect:
        itauAuditable.compare(left, right).hasChanges() == false
    }

    @Unroll
    def "should use ReferenceChange when #propType is annotated as ShallowReferences"() {
        given:
        def itauAuditable = itauAuditable().withMappingStyle(mapping).build()
        ...
        when:
        def changes = itauAuditable.compare(left, right).getChangesByType(ReferenceChange)
    ...
    def "should not compare properties when #propType is annotated as ShallowReference"() {
        ...
        when:
        def changes = itauAuditable.compare(left, right).changes
    ...
    def "should ignore properties with @DiffIgnored type"() {
        ...
        then:
        diff.changes.size() == 0
        itauAuditable.getTypeMapping(DummyIgnoredType) instanceof IgnoredType
        itauAuditable.getTypeMapping(IgnoredSubType) instanceof IgnoredType
    }
    ...
    def "should ignore properties with type explicitly registered as ignored" () {
        given:
        def itauAuditable = itauAuditable().registerIgnoredClass(Bar).build()
        ...
        then:
        diff.changes.size() == 0
        itauAuditable.getTypeMapping(Bar) instanceof IgnoredType
    }

    def "should ignore properties with type registered as ignored using IgnoredClassesStrategy" () {
        given:
        def itauAuditable = itauAuditable().registerIgnoredClassesStrategy(
                {c -> return c == Bar}
        ).build()
        ...
        itauAuditable.getTypeMapping(Bar) instanceof IgnoredType
        itauAuditable.getTypeMapping(BarBar) instanceof ValueObjectType
    }
    ...
    def "should compare ValueObjects in Lists as Sets when ListCompareAlgorithm.SET is enabled"() {
      given:
      def itauAuditable = itauAuditable().withListCompareAlgorithm(ListCompareAlgorithm.AS_SET).build()
      ...
      def s2 = new SnapshotEntity(id: 1, listOfValueObjects: [
       ...
       when:
       def diff = itauAuditable.compare(s1, s2)
       println diff
       ...
       def expectedAddedId = SnapshotEntity.class.name + "/1#listOfValueObjects/"+
               itauAuditableTestAssembly().hash(new DummyAddress("Warsaw", "some"))
    ...
    def "should report when a list property is added or removed"(){
      ...
      when:
      diff = itauAuditable.compare(object2, object1)
      println diff.prettyPrint()
