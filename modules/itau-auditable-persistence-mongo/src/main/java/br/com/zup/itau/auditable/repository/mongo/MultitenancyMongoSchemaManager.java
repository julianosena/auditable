...
class MultitenancyMongoSchemaManager {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MultitenancyMongoSchemaManager.class);

    private final MongoClient mongoClient;
    private final String DATABASE_NAME_DEFAULT;

    MultitenancyMongoSchemaManager(MongoClient mongoClient, String databaseNameDefault) {
        this.mongoClient = mongoClient;
        this.DATABASE_NAME_DEFAULT = databaseNameDefault;
    }
    ...
    public void ensureSchema(MongoDialect dialect) {
        ...
        if (doc != null) {
            ...
            if (stringCommitId instanceof String) {
                ...
                Document ret = this.getMongoDatabase().runCommand(update);
                logger.info("result: \n "+ ret.toJson());
            }
        }
    }
    ...
    MongoCollection<Document> snapshotsCollection() {
        return this.getMongoDatabase().getCollection(SNAPSHOTS);
    }

    MongoCollection<Document> headCollection() {
        return this.getMongoDatabase().getCollection(MongoHeadId.COLLECTION_NAME);
    }
