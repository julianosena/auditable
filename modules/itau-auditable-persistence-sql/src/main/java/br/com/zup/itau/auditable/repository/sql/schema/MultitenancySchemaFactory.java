...
public class MultitenancySchemaFactory extends MultitenancySchemaNameAware {
    ...
    public MultitenancySchemaFactory(Dialect dialect, MultitenancyTableNameProvider tableNameProvider) {
    ...
    private Schema snapshotTableSchema(Dialect dialect){
        MultitenancyDBObjectName tableName = getSnapshotTableName();
    ...
    private Schema commitTableSchema(Dialect dialect) {
        MultitenancyDBObjectName tableName = getCommitTableName();
        Schema schema = emptySchema(dialect);
        RelationBuilder relationBuilder = schema.addRelation(tableName.localName());
    ...
    private Schema commitPropertiesTableSchema(Dialect dialect) {
        MultitenancyDBObjectName tableName = getCommitPropertyTableName();
        Schema schema = emptySchema(dialect);
        RelationBuilder relationBuilder = schema.addRelation(tableName.localName());
        relationBuilder
    ...
    private Schema globalIdTableSchema(Dialect dialect){
        MultitenancyDBObjectName tableName = getGlobalIdTableName();
    ...
    Schema emptySchema(Dialect dialect) {
        return new Schema(dialect, getSchemaName());
    }

    private void foreignKey(MultitenancyDBObjectName tableName, String fkColName, boolean isPartOfPrimaryKey, String targetTableName, String targetPkColName, RelationBuilder relationBuilder){
    ...
    private void columnsIndex(MultitenancyDBObjectName tableName, Schema schema, String... colNames){
        columnsIndex(tableName, schema, new IndexedCols(colNames));
    }

    private void columnsIndex(MultitenancyDBObjectName tableName, Schema schema, IndexedCols indexedCols){
    ...
    String createIndexName(MultitenancyDBObjectName tableName, IndexedCols indexedCols) {
    ...
    private void primaryKey(String pkColName, Schema schema, RelationBuilder relationBuilder, String seqNameLocal) {
        relationBuilder.withAttribute().longAttr(pkColName).withAdditionalModifiers("AUTO_INCREMENT").notNull().and()
                .primaryKey("audit_"+pkColName).using(pkColName).and();
